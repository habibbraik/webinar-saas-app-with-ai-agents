      ...
      <DialogContent className="sm:max-w-md bg-card text-card-foreground border-border">
        ...
        <DialogFooter className="flex justify-between items-center mt-4 sm:mt-0">
          ...
          >
            {loading ? (
              <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" /> {/* import from "lucide-react" */}
                Loading...
              </>
            ) : webinar?.ctaType === 'BOOK_A_CALL' ? (
              'Join Break-room'
...
function createCheckoutLink(priceId: string, stripeConnectId: string, userId: string, id: string, arg4: boolean) {
